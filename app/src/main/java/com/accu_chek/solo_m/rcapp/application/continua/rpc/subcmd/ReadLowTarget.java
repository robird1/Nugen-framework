 ...
 * ===========================================================================
 *
 * Class name: com.accu_chek.solo_m.rcapp.application.continua.rpc.subcmd.ReadLowTarget
 * Brief: 
 *
 * Create Date: 2015/7/1
 * $Revision: 22494 $
 * $Author: kevenwu $
 * $Id: ReadLowTarget.java 22494 2015-10-26 03:35:47Z kevenwu $
 */
...
public class ReadLowTarget implements IRPCCommandHandler
{
    /**
     * This function retrieves the low target from Bolus time block database and return to Agent.
    ...
    public void handle(ContinuaCommandSet commandSet, RPCCommandInvocation invocation)
    {
        ...
        if (null != patientRecord)
        {
            ...
            SafetyChannel<Integer> lowValue = timeblock.getBgLowerTarget();
            
            int lowTarget = CommonUtils.getOriginValue(lowValue.getValueCH1(), lowValue.getValueCH2());
            byte[] lowInBytes = ParseUtils.makeLittleEndian(ParseUtils.parseInt16(lowTarget));
            
            RPCDataArguments argument = new RPCDataArguments();
            
            argument.setType(RPCArgumentType.RPC_ARG_TYPE_UINT16);
            argument.setLength(new SafetyNumber<Integer>(lowInBytes.length, -lowInBytes.length));
            argument.setValue(new SafetyByteArray(lowInBytes, CRCTool.generateCRC16(lowInBytes)));
