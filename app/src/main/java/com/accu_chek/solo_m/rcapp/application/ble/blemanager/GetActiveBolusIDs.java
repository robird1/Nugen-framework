 ...
 * ===========================================================================
 *
 * Class name: ccom.accu_chek.solo_m.rcapp.application.ble.blemanager.GetActiveBolusIDs
 * Brief: This class handles GetActiveBolusIDs request and its response.
 *
 * Create Date: 2015/10/13
 * $Revision: 21344 $
 * $Author: KiddYeh $
 * $Id: GetActiveBolusIDs.java 21344 2015-10-12 12:12:13Z KiddYeh $
 */
...
public class GetActiveBolusIDs implements IBLERequest
{
    private static final String TAG = "GetActiveBolusIDs";
    
    // Two Byte Operand
    private static final int TWO_BYTES_OPERAND = 0xFFFF;
    
    /**
     * The instance of GetActiveBolusIDs class
     */
    private static volatile GetActiveBolusIDs mInstance = null;
    ...
    /**
     * This method gets the one and only instance of the class GetActiveBolusIDs.
     * 
     ...
     *            Scaling: 1
     * 
     * @return Scan : the one and only instance of the class GetActiveBolusIDs
     *         Range: A valid object of GetActiveBolusIDs
     *         Unit: GetActiveBolusIDs
     *         Scaling: 1
     * 
     * @see mInstance: use this global variable to store the instance of the
     *      class GetActiveBolusIDs
     */
    public static GetActiveBolusIDs getInstance(Context context)
    {
        if (null == mInstance)
        {
            mInstance = new GetActiveBolusIDs(context);
        }
        else
        {
     ...
     *            
     */
    private GetActiveBolusIDs(Context context)
    {
        mContext = context;
    }
    ...
    /**
     * This method gets the cause of response to check if the GetActiveBolusIDs is successful or not.
    ...
    {
        Debug.printD(TAG, "[GetActiveBolusIDs]: Response enter ==> WriteResponse");
        ...
        {   
            ...
            Debug.printD(TAG, "[GetActiveBolusIDs]: WriteResponse ==> failed ");
        }
        else
        {
            Debug.printD(TAG, "[GetActiveBolusIDs]: WriteResponse ==> OK ");
            // Apply to the coding standard    
        }
     ...
     * @see mCallback 
     */
    protected void requestGetActBolusIDsCP(byte[] data)
    {   
    ...
    public void request(BLERequestParameter parameter, ResponseCallback callback)
    {
        ...
        byte[] data = {(byte) OpCode.IDSREAD_ACTIVE_BOLUS_ID,(OpCode.IDSREAD_ACTIVE_BOLUS_ID >> 8)};
    ...
    class AttrNotification implements ResponseProcess
    {
        ...
        public void doProcess(Context context, Intent intent)
        {
            Debug.printD(TAG, "[GetActiveBolusIDs]: Response enter ==> AttrNotification");
           ...
           // Get E2E Result
            ...
            switch(iOpCode)
            {
            case ControlPointConstant.OpCode.IDSREAD_ACTIVE_BOLUS_ID_RESP:

                if (iE2EResult == E2E_Result.RESULT_OK)
                {
                    isResult = SafetyBoolean.TRUE;
                    Debug.printD(TAG, " [GetActiveBolusIDs]: E2E_Result.RESULT_OK");
                }
                else
                {
                    isResult = SafetyBoolean.FALSE;
                    Debug.printD(TAG, " [GetActiveBolusIDs]: E2E_Result.RESULT_NNNNNNNNNNNNNNNNNNNNNOK");
                }
            ...
            case ControlPointConstant.OpCode.IDSREAD_RESP:
                ...
                if (iCommandCode == 
                        ControlPointConstant.OpCode.IDSREAD_ACTIVE_BOLUS_ID)
                {
                    Debug.printD(TAG, " [GetActiveBolusIDs]: STD CMD Error");
                    isResult = SafetyBoolean.FALSE;
                    attPack = null;
                }
